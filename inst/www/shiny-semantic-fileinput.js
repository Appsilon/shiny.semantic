...
var SemanticFileProcessor = function(files) {
  this.files = files;
  this.fileIndex = -1;
...
}).call(SemanticFileProcessor.prototype);

var SemanticFileUploader = function(shinyapp, id, files, el) {
  this.shinyapp = shinyapp;
  this.id = id;
  this.el = el;
  SemanticFileProcessor.call(this, files);
};
$.extend(SemanticFileUploader.prototype, SemanticFileProcessor.prototype);
(function() {
  ...
  this.onComplete = function() {
    ...
    this.makeRequest(
      ...
      function(response) {
        ...
        self.$bar().progress('set label', 'Upload complete');
    ...
    this.$bar().progress('set label', 'Finishing upload');
  };
  ...
  this.onProgress = function(file, completed) {
    this.$bar().progress('set percent', Math.round(completed * 100));
    this.$bar().progress('set label', file ? file.name : '');
  };
  this.$container = function() {
    return $('#' + Shiny.$escape(this.id) + '_progress.ui-ss-progress-file');
  };
  this.$bar = function() {
    return $('#' + Shiny.$escape(this.id) + '_progress.ui-ss-progress-file');
  };
  ...
  this.$setError = function(error) {
    var errorState = (this.$bar().progress('is error') ) + ' error';
    this.$bar().toggleClass('error', (error !== null));
    if (error !== null) {
      this.onProgress(null, 1);
      this.$bar().progress('set error', error);
    }
  };
  this.$setActive = function(active) {
    this.$container().toggleClass('active', !!active);
  };
}).call(SemanticFileUploader.prototype);
...
function setFileText($el, files) {
  var $fileText = $el.closest('div.ui-ss-input').find('input[type=text]');
  if (files.length === 1) {
...
function uploadDroppedFilesIE10Plus(el, files) {
  ...
  $el.data(
    'currentUploader',
    new SemanticFileUploader(Shiny.shinyapp, semanticFileInput.getId(el), files, el)
  );
}
...
function uploadFiles(evt) {
  ...
  var id = semanticFileInput.getId(evt.target);
  ...
  $el.data('currentUploader',
    new SemanticFileUploader(Shiny.shinyapp, id, files, evt.target));
}
...
var semanticFileInput = new Shiny.InputBinding();
$.extend(semanticFileInput, {
  find: function(scope) {
  ...
  getValue: function(el) {
    ...
    if (data) {
      ...
      // code from SemanticFileUploader, but duplication is less bad than alternatives.
  ...
  _zoneOf: function(el) {
    return $(el).closest("div.ui-ss-input");
  },
  ...
  subscribe: function(el, callback) {
    $(el).on("change.semanticFileInput", uploadFiles);
...
Shiny.inputBindings.register(semanticFileInput, 'shiny.semanticFileInput');
