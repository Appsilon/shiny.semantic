...
create_example <- function(code, ...) {
  list(
    htmlwidget = shiny.semantic::uirender(code),
    code_demo = highlighter::highlight(formatR::tidy_source(output = FALSE, width.cutoff = 80, text = deparse(substitute(code)))$text.tidy)
  )
}
```
...
```{r, echo = FALSE}
example_button = create_example(div(class = "ui basic button", uiicon("user"),  "Icon button"))
