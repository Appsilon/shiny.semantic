...
#' @param  color Color of the box. Find available colors in \code{COLOR_PALETTE}.
#' @param ribbon Should label be presented as ribbon.
#' @param title_side Side of a label. One of \code{c("top", "bottom", "top left",
#' "top right", "bottom left", "bottom right")} if \code{ribbon = FALSE}, or
#' one of \code{c("top left", "top right")} if \code{ribbon = TRUE}
...
#' )
#' }
box <- function(..., title = NULL, color = "red", ribbon = TRUE, title_side = "top right",
  ...
  js_script <- glue::glue("$('#{box_id}').accordion({{
    ...
    onOpening: function() {{ $('#{box_id}').find('.label .icon').
                   removeClass('{expand_icon}').addClass('{collapse_icon}'); }},
    onClosing: function() {{ $('#{box_id}').find('.label .icon').
